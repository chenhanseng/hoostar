...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuewendu_value != cur->valuewendu_value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuewendu_value Changed\n");
            ret = 1;
        }
    }
    if(last->valueshidu_value != cur->valueshidu_value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueshidu_value Changed\n");
            ret = 1;
        }
    }
    if(last->valueV_value != cur->valueV_value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueV_value Changed\n");
            ret = 1;
        }
    }
    if(last->valuesmoke_value != cur->valuesmoke_value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesmoke_value Changed\n");
            ret = 1;
        }
    }
    if(last->valueCO2_value != cur->valueCO2_value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueCO2_value Changed\n");
            ret = 1;
        }
    }
    if(last->valueTVOC_value != cur->valueTVOC_value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTVOC_value Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valuewendu_value = gizY2X(wendu_value_RATIO,  wendu_value_ADDITION, dataPoints->valuewendu_value); 
    devStatusPtr->valueshidu_value = gizY2X(shidu_value_RATIO,  shidu_value_ADDITION, dataPoints->valueshidu_value); 
    devStatusPtr->valueV_value = gizY2X(V_value_RATIO,  V_value_ADDITION, dataPoints->valueV_value); 

    devStatusPtr->valuesmoke_value = exchangeBytes(gizY2X(smoke_value_RATIO,  smoke_value_ADDITION, dataPoints->valuesmoke_value)); 
    devStatusPtr->valueCO2_value = exchangeBytes(gizY2X(CO2_value_RATIO,  CO2_value_ADDITION, dataPoints->valueCO2_value)); 
    devStatusPtr->valueTVOC_value = exchangeBytes(gizY2X(TVOC_value_RATIO,  TVOC_value_ADDITION, dataPoints->valueTVOC_value)); 
